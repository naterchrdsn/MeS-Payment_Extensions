...
if($_POST) // Run transaction
{
  ...
  if($transaction->ResponseFields['error_code'] == "000") //Approval
  {
    ...
    $mes->validateOrder($cart->id, _PS_OS_PAYMENT_, $transaction_amount, "Merchant e-Solutions", $admin_note, $mail_vars, $cookie->id_currency, false, $customer->secure_key);
	//$aim->validateOrder($cart->id, Configuration::get('PS_OS_PAYMENT'), $response[9], $payment_method, $message, NULL, NULL, false, $customer->secure_key);
