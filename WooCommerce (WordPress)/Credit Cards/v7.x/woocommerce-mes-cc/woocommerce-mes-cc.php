...
function init_mes_cc() {
	 ...
	 */
	class WC_Gateway_Mes_CC extends WC_Payment_Gateway_CC {
		...
		public function payment_fields() {
			...
			if ( 'pg' == $this->mode ) {
				$this->form( array( 'fields_have_names' => true ) );
			}
		}
		...
		protected function process_pg_payments( $order ) {
			if (!isset($this->profile_id) || !isset($this->profile_key)) {
				...
				$tran->setAvsRequest($order->get_billing_address_1(), $order->get_billing_postcode());
				$tran->setRequestField('cvv2', $_POST['mes_cc-card-cvc']);
				$tran->setRequestField('invoice_number', $order->get_id());
				$tran->setRequestField('currency_code', strtoupper( get_woocommerce_currency() ));
				$tran->setRequestField('dest_country_code', $order->get_shipping_country());
				$tran->setRequestField('ship_to_zip', $order->get_shipping_postcode());
				$tran->setRequestField('ship_to_address', $order->get_shipping_address_1());
				$tran->setRequestField('ship_to_last_name', $order->get_shipping_last_name());
				$tran->setRequestField('ship_to_first_name', $order->get_shipping_first_name());
				$tran->setRequestField('country_code', $order->get_billing_country());
				$tran->setRequestField('custom', $order->get_id());
				$tran->setRequestField('cardholder_first_name', $order->get_billing_first_name());
				$tran->setRequestField('cardholder_last_name', $order->get_billing_last_name());
				$tran->setRequestField('cardholder_email', $order->get_billing_email());
				$tran->setRequestField('cardholder_phone', $order->get_billing_phone());
				$tran->setRequestField('account_name', $order->get_user_id());
				$tran->setRequestField('account_email', $order->get_billing_email());
				$tran->setTransactionData(str_replace(' ', '', $_POST['mes_cc-card-number']), str_replace(' / ', '', $_POST['mes_cc-card-expiry']), $order->get_total());
				$tran->execute();
		...
		protected function get_hosted_payments_args( $order ) {
			$args = apply_filters( 'woocommerce_mes_payhere_args', array(
				...
				'use_merch_receipt'   => 'Y',
				'cardholder_street_address' => $order->get_billing_address_1(),
				'cardholder_zip'     => $order->get_billing_postcode(),
			...
			if (isset($this->test_mode)) {
				$args['use_simulator'] = 'Y';
			}
			if (isset($this->security_key)) {
				$tran_key = md5($this->profile_key.$this->security_key.$order->get_total());
