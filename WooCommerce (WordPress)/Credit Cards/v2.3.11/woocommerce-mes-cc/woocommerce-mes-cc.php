...
function init_mes_cc() {
	...
	class WC_Gateway_Mes_CC extends WC_Payment_Gateway {
		...
		public function __construct() {
			...
			$this->merch_email      = $this->get_option( 'merch_email' );
		...
		public function checks() {
			...
			elseif ( ! $this->profile_id || ! ( ($this->profile_key && 'pg' == $this->mode) || ($this->merch_email && 'ph' == $this->mode) ) ) {
		...
		public function admin_options() {
			...
			<?php if ( empty( $this->profile_id ) ) : ?>
				<div class="mes-banner updated">
					...
					<p><a href="https://developer.merchante-solutions.com/#/sandbox-signup" target="_blank" class="button button-primary"><?php _e( 'Sign up with Merchant e-Solutions', 'woocommerce' ); ?></a> <a href="https://www.merchante-solutions.com" target="_blank" class="button"><?php _e( 'Learn more', 'woocommerce' ); ?></a></p>

				</div>
			<?php else : ?>
				<p><?php _e( 'This is an area we can put nice filler text that describes merchant e-solutions payment solutions', 'woocommerce' ); ?></p>
			<?php endif; ?>
			...
			<table class="form-table">
				...
				<script type="text/javascript">
					jQuery( '#woocommerce_mes_cc_mode' ).on( 'change', function() {
						var key = jQuery( '#woocommerce_mes_cc_profile_key' ).closest( 'tr' ),
							email = jQuery( '#woocommerce_mes_cc_merch_email' ).closest( 'tr' );
							tran = jQuery( '#woocommerce_mes_cc_tran_type' ).closest( 'tr' );
					...
					jQuery( "#woocommerce_mes_cc_merch_email" ).attr( "maxlength", 50 );
		...
		public function init_form_fields() {
			$this->form_fields = array(
				...
				'mode' => array(
					...
					'type'        => 'select',
					'description' => sprintf( __( 'Choose Payment Gateway to use the MeS Payment Gateway API via a regular credit card form displayed to your customers (SSL required). %1$s Choose PayHere to use the MeS PayHere API and redirect the customer to the MeS PayHere hosted page (if not SSL). %1$s Note: You must have either Payment Gateway or PayHere API access on your account!', 'woocommerce' ), '<br />' ),
					'default'     => 'pg',
					'options'     => array(
				...
				'merch_email' => array(
					'title'		  => __( 'Merchant E-mail', 'woocommerce' ),
					'type'        => 'email',
					'description' => __( 'This is the primary email on your merchant account with Merchant e-Solutions.', 'woocommerce' ),
					'default'     => '',
					'desc_tip'    => true
				),
			);
		...
		protected function get_hosted_payments_args( $order ) {
			$tran_key = md5('EtLwsVwzcecgpHqkrNjhXPVyvUnEYHdn'.'1234'.$order->order_total);
		...
		public function process_order_status( $order, $payment_id, $status, $auth_code ) {
			if ( '000' == $status ) {
				...
				// Add order note
				$order->add_order_note( sprintf( __( 'MeS PG payment approved (Transaction ID: %s, Auth Code: %s)', 'woocommerce' ), $payment_id, $auth_code ) );
